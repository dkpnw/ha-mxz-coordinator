...
An optional per-room "airflow" (``stage``) sensor keeps the tile's fan display
...
class MXZRoomClimate(MXZEntity, CoordinatorEntity[MXZCoordinator], ClimateEntity):
    ...
    def __init__(
        ...
        self._stage_sensor_id = (
            coordinator.primary_stage_sensor_id
            if primary
            else coordinator.secondary_stage_sensor_id
        )

...
async def test_restart_adopts_a_rung_held_by_hysteresis(hass: HomeAssistant) -> None:
    ...
    state. v2.19.0 compared the seed against the cold read only, so a restart
    here still latched the head. The seed check now accepts any rung the ladder
    would STAY on at the current delta.
    """
    hass.config.units = US_CUSTOMARY_SYSTEM
    head_a, head_b = await _setup_mock_heads(hass)
    await _set_temp(hass, SENSOR_A, 70)
    ...
    await _recompute(hass, entry)
    plan = hass.states.get(_eid(hass, entry, "_plan"))
    assert plan.attributes["primary_fan_hold"] is False  # adopted, not held
    ...
    await _recompute(hass, entry)
    plan = hass.states.get(_eid(hass, entry, "_plan"))
    assert plan.attributes["primary_fan_hold"] is True  # genuine hold kept
...
async def test_restart_mid_boost_keeps_boost_driving(hass: HomeAssistant) -> None:
    ...
    update. Observed live: a head at "low" came back held while its neighbour at
    the top token self-healed (the old adopt only covered max).
...
async def test_max_hold_survives_a_target_move(hass: HomeAssistant) -> None:
    ...
    await _recompute(hass, entry)
    plan = hass.states.get(_eid(hass, entry, "_plan"))
    assert plan.attributes["primary_fan_hold"] is True  # no self-release
    assert hass.states.get(head_a).attributes["fan_mode"] == "high"
...
async def test_max_hold_stays_latched_when_delta_grows_only_partway(
    ...
    await _recompute(hass, entry)
    plan = hass.states.get(_eid(hass, entry, "_plan"))
    assert plan.attributes["primary_fan_hold"] is True  # not max -> still held
...
async def test_engage_deadband_clamped_to_profile_bounds(hass: HomeAssistant) -> None:
    """Hand-edited re-engage drift clamps into the sane range (0.5-5 F)."""
    hass.config.units = US_CUSTOMARY_SYSTEM
    head_a, head_b = await _setup_mock_heads(hass)
    await _set_temp(hass, SENSOR_A, 70)
    await _set_temp(hass, SENSOR_B, 70)
    for raw, expected in ((40.0, 5.0), (0.0, 0.5)):
        entry = MockConfigEntry(
            domain=DOMAIN,
            title="MXZ Coordinator",
            data={

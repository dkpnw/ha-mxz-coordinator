...
- **Or hand it back from the slider.** Set the fan to `auto` and boost resumes on the next
  ...
  included, holds. (Through v3.0.0-beta.17, sliding to max while boost was already running
  flat out handed control back too. That was the escape hatch for HomeKit, whose fan slider
  has no `auto` stop; the **Fan auto** switch is the escape hatch now, and it's unambiguous
  where a slide to max never was — it could equally mean "give me more air".)
